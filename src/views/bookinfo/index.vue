...
<!--**************************-->
    <el-table-column
          ...
          width="100">
        <template slot-scope="scope">
          {{ scope.row.booknum - scope.row.bookout }}
        </template>
      </el-table-column>
      <el-table-column
    ...
    **************************-->
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button @click="handleBorrow(scope.row)" type="success" size="small">收藏图书</el-button>
        </template>
      </el-table-column>
      <!--开始改动-->
      <!-- <el-table-column
  ...
  methods: {
    ...
    handleBorrow(row) {
      if (this.roleIsAdmin) {
        ...
        this.$confirm('您确定要收藏图书吗?', '提示').then(() => {
          borrowBook(this.id, row.bookid).then(res => {
            if(res === 1) {
              this.$message.success('收藏图书成功')
