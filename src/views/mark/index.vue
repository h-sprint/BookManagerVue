    ...
    <el-table
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <el-button v-if="scope.row.returntimestr === null || scope.row.returntimestr === ''" @click="handleReturn(scope.row,scope.$index)" type="success" size="small">取消收藏图书</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { getCount,queryMarks,queryMarksByPage,addMark,deleteMark,deleteMarks,updateMark,returnBook } from '@/api/Mark'
  ...
  methods: {
      ...
      this.$confirm('确定要删除该条记录吗?', '提示', {
        ...
        deleteMark(row).then(res => {
          if(res === 1) {
            ...
            this.$message.error('存在未完成的收藏信息，删除失败')
          }
        })
      })
    },
    ...
    handleReturn(row, index) {
      this.$confirm('确定要取消收藏吗?', '提示', {
        ...
        returnBook(row.markid, row.markid).then(res => {
          if(res === 1) {
            this.$message.success('取消收藏成功')
            this.handleCurrentChange(this.queryParam.page)
          } else if(res === 0){
            this.$message.error('该图书已经是取消收藏的状态')
          } else {
